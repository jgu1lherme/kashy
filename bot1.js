...
const { random, sample } = require('lodash'); // Usado para respostas personalizadas de sucesso

// --- Configuração ---
// const GASTOS_FILE_PATH = path.join(__dirname, 'gastos.txt'); // Não usamos mais um único arquivo
const GASTOS_DATA_DIR = path.join(__dirname, 'gastos_data'); // Novo diretório para dados por chat
const SESSION_DIR = path.join(__dirname, 'baileys_auth_info');


// --- Estado e Armazenamento ---
const store = makeInMemoryStore({ logger: pino().child({ level: 'silent', stream: 'store' }) });
// Tentativa de carregar o estado da store (geral)
store?.readFromFile(path.join(SESSION_DIR, 'baileys_store.json'));
// Salva o estado da store periodicamente
setInterval(() => {
    store?.writeToFile(path.join(SESSION_DIR, 'baileys_store.json'));
}, 10_000);

// Objeto para guardar o estado da confirmação por usuário (já é por chatId)
let userConfirmationState = {};

// Objeto para armazenar dados da última despesa adicionada para confirmação de categoria (já é por chatId)
...
function getRandomSuccessPhrase(value, description) {
     // Usamos sample do lodash para pegar um item aleatório de um array
    const phrase = sample(successPhrases); // Corrigido para usar sample
    return phrase
...
// --- Helper function to get file path for a given chatId ---
const getGastosFilePath = (chatId) => {
    // Cria um nome de arquivo seguro a partir do chatId (geralmente o chatId já é seguro)
    // Podemos adicionar .txt no final
    const fileName = `${chatId}.txt`;
    return path.join(GASTOS_DATA_DIR, fileName);
};


// --- Helper function to read and parse expenses for a specific chatId ---
// Formato: timestamp;value;description;category\n
const readExpensesFromFile = (chatId) => {
    const filePath = getGastosFilePath(chatId);
    if (!fs.existsSync(filePath)) {
        return []; // Return empty array if file doesn't exist for this chat
    }
...
// Helper function to write expenses back to file for a specific chatId (used for category updates)
const writeExpensesToFile = (chatId, expenses) => {
    const filePath = getGastosFilePath(chatId);
    const lines = expenses.map(exp =>
        `${exp.timestamp.toISOString()};${exp.value.toFixed(2)};${exp.description};${exp.category}`
    );
    // Ensure the data directory exists before writing
    if (!fs.existsSync(GASTOS_DATA_DIR)){
        fs.mkdirSync(GASTOS_DATA_DIR, { recursive: true });
    }
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (userConfirmationState[chatId]) {
             ...
             if (pending.type === 'expense_confirm') {
                 if (lowerCaseText === 'sim' || lowerCaseText === 's') {
                    ...
                    try {
                        // --- Tenta salvar o gasto no arquivo ESPECÍFICO DESTE CHAT ---
                        const record = `${timestamp.toISOString()};${pending.value.toFixed(2)};${pending.description};Outros\n`;
                         const filePath = getGastosFilePath(chatId); // Obtém o caminho do arquivo
                         // Garante que o diretório exista antes de escrever
                         if (!fs.existsSync(GASTOS_DATA_DIR)){
                             fs.mkdirSync(GASTOS_DATA_DIR, { recursive: true });
                         }
                        fs.appendFileSync(filePath, record); // Salva no arquivo correto
                        fileSavedSuccessfully = true; // Mark success
                    } catch (fileSaveError) {
                        // --- Captura ERROS APENAS do salvamento do arquivo ---
                        console.error(`[${chatId}] Erro REAL ao salvar no arquivo:`, fileSaveError); // Log com chatId
                    ...
                    if (fileSavedSuccessfully) {
                        try {
                            ...
                            console.error(`[${chatId}] Erro ao enviar mensagem de confirmação de categoria:`, sendMessageError); // Log com chatId
                  ...
                  if (!expenseData) {
                      console.error(`[${chatId}] lastAddedExpenseData missing for category_confirm.`); // Log
                  ...
                  if (lowerCaseText === 'sim' || lowerCaseText === 's') {
                      ...
                      try {
                         // Lê os gastos APENAS deste chat
                         let expenses = readExpensesFromFile(chatId);
                         let updated = false;
                         ...
                         if (updated) {
                              // Escreve os gastos atualizados APENAS deste chat
                             writeExpensesToFile(chatId, expenses);
                          ...
                          console.error(`[${chatId}] Erro ao atualizar categoria no arquivo:`, err); // Log com chatId
                 ...
                 if (lowerCaseText === 'sim' || lowerCaseText === 's') {
                      try {
                         const filePath = getGastosFilePath(chatId); // Obtém o caminho do arquivo deste chat
                         if (fs.existsSync(filePath)) {
                              const fileContent = fs.readFileSync(filePath, 'utf-8');
                             if (fileContent.trim().length > 0) {
                                 fs.unlinkSync(filePath); // Apaga o arquivo ESPECÍFICO deste chat
                         ...
                         console.error(`[${chatId}] Erro ao apagar arquivo:`, err); // Log com chatId
                 ...
                 console.warn(`[${chatId}] Unhandled confirmation state type: ${pending.type}`); // Log com chatId
        ...
        if (lowerCaseText === '/ajuda') {
            const helpMessage = `🤖 *Comandos Disponíveis:*\n\n` +
                                ...
                                `📊 *Relatórios:*\n` +
                                `   \`/relatorio\` - Mostra todos os gastos por mês/dia, totais mensais e ranking de categorias DESTE CHAT.\n` +
                                `   \`/hoje\` - Mostra apenas os gastos de hoje DESTE CHAT com total.\n` +
                                `   \`/semana\` - Mostra os gastos dos últimos 7 dias DESTE CHAT com total.\n` +
                                `   \`/total\` - Mostra o valor total de todos os gastos registrados DESTE CHAT.\n\n` +
                                `🗑️ *Gerenciamento:*\n` +
                                `   \`/limpar\` - Apaga TODOS os gastos registrados DESTE CHAT (pedirá confirmação). Responda "sim" ou "não".\n\n` +
         ...
         if (lowerCaseText === '/total') {
            try {
                // Lê gastos APENAS deste chat
                const expenses = readExpensesFromFile(chatId);
                if (expenses.length === 0) {
                    await sock.sendMessage(chatId, { text: '📂 Nenhum gasto encontrado ainda para este chat.' });
                    return;
                }

                const total = expenses.reduce((sum, expense) => sum + expense.value, 0);
                const totalFormatted = total.toFixed(2).replace('.', ',');
                await sock.sendMessage(chatId, { text: `📈 *Total de Gastos Registrados (Este Chat):*\n\n💸 R$${totalFormatted}` });

            } catch (err) {
                console.error(`[${chatId}] Erro ao calcular total:`, err); // Log com chatId
         ...
         if (lowerCaseText === '/hoje') {
            try {
                ...
                if (todayExpenses.length === 0) {
                    await sock.sendMessage(chatId, { text: '🗓️ Nenhum gasto registrado para hoje neste chat.' });
                    return;
                }

                let reportMessage = `🗓️ *Gastos de Hoje - ${today.toLocaleDateString('pt-BR')} (Este Chat)*\n\n`;
                 let totalToday = 0;
                ...
                console.error(`[${chatId}] Erro ao gerar relatório de hoje:`, err); // Log com chatId
            ...
            try {
                ...
                sevenDaysAgo.setHours(0, 0, 0, 0); // Ensure start of the day
                 ...
                 if (weekExpenses.length === 0) {
                    await sock.sendMessage(chatId, { text: '🗓️ Nenhum gasto registrado nos últimos 7 dias neste chat.' });
                    return;
                }

                 let reportMessage = `🗓️ *Gastos dos Últimos 7 Dias (Este Chat):*\n\n`;
                ...
                console.error(`[${chatId}] Erro ao gerar relatório da semana:`, err); // Log com chatId
        ...
        // /relatorio - Show full report
         if (lowerCaseText === '/relatorio') {
            try {
                ...
                let reportMessage = `📊 *Relatório de Gastos (Este Chat):*\n\n`;
                ...
                if (sortedCategories.length > 0) {
                     reportMessage += `🏆 *Ranking de Categorias (Este Chat):*\n`;
                ...
                console.error(`[${chatId}] Erro ao gerar relatório:`, err); // Log com chatId
         ...
         if (lowerCaseText === '/limpar') {
             // Obtém o caminho do arquivo deste chat
             const filePath = getGastosFilePath(chatId);

            // Check if there are actually expenses to clear for this chat
             if (!fs.existsSync(filePath) || fs.readFileSync(filePath, 'utf-8').trim().length === 0) {
                  await sock.sendMessage(chatId, { text: '📂 Nenhum gasto encontrado para apagar neste chat.' });
                  return;
             }
            ...
            const clearConfirmationMessage = '⚠️ *ATENÇÃO:* Você tem certeza que deseja apagar TODOS os gastos registrados **deste chat**?\nEsta ação não pode ser desfeita.\n\n_(Responda com "sim" para confirmar ou "não" para cancelar)_';
...
// Ensure the data directory and session directory exist before starting
if (!fs.existsSync(GASTOS_DATA_DIR)){
    fs.mkdirSync(GASTOS_DATA_DIR, { recursive: true });
    console.log(`Diretório de dados criado: ${GASTOS_DATA_DIR}`);
}
